...
class pySOT_class_dict():
	...
	def tail_generate_dict(self):
		...
		self.tail_dict = self.get_class_names(mod_name, mod_name.__name__)
	...
	def optimization_problem_generate_dict(self):
		...
		self.optimization_problem_dict = self.get_class_names(mod_name, mod_name.__name__)
		for i in self.optimization_problem_dict:
			self.optimization_problem_dict[i] = self.get_arguments_and_default_values(self.optimization_problem_dict[i])
	...
	def experimental_design_generate_dict(self):
		...
		self.experimental_design_dict = self.get_class_names(mod_name, mod_name.__name__)

		for i in self.experimental_design_dict:
			self.experimental_design_dict[i] = self.get_arguments_and_default_values(self.experimental_design_dict[i])
	...
	def surrogate_model_generate_dict(self):
		...
		self.surrogate_model_dict = self.get_class_names(mod_name, mod_name.__name__)

		for i in self.surrogate_model_dict:
			self.surrogate_model_dict[i] = self.get_arguments_and_default_values(self.surrogate_model_dict[i])
	...
	def adaptive_sampling_generate_dict(self):
		...
		self.adaptive_sampling_dict = self.get_class_names(mod_name, mod_name.__name__)
		for i in self.adaptive_sampling_dict:
			self.adaptive_sampling_dict[i] = self.get_arguments_and_default_values(self.adaptive_sampling_dict[i])
		print(' ')
		print(self.adaptive_sampling_dict)

	def get_dict(self):
		return { 'kernel': self.kernel_dict, 'tail' : self.tail_dict, 'optimization_problem' : self.optimization_problem_dict, 'experimental_design' : self.experimental_design_dict, 'surrogate_model' : self.surrogate_model_dict , 'adaptive_sampling' : self.adaptive_sampling_dict}
